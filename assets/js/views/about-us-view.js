...
((m) => {
    ...
    class AboutUsPage {
        ...
        async onPageActive() {
            ...
            if (!this._contentFetched) {
                try {
                    const response = await fetch(window.location.origin + "/api/get-about-us")
                    .then(res => res.json())
                    .then(d => { 
                        if (d.status == "success") 
                            return d.content; 
                    ...
                    this._aboutUsContent = `<div class="about-us-container">${response}</div>`;
                    this._contentFetched = true;
                } catch (ex) {
                    console.log("qualcosa è andata storta.", ex);
                    this._aboutUsContent = ABOUT_US_HTML;
                }
            }
            
            if (!this._aboutUsContent)
                this._aboutUsContent = ABOUT_US_HTML;
                
            this._targetElement.innerHTML = this._aboutUsContent; 
        }
