((m) => {
    ...
    class RadioCoverImage extends CustomComponent {
        constructor({cover}) {
            super();
        ...
        onRender() {
            const element = this.getViewElement();
            if (this.coverImage && this.coverImage.length > 0 && element) {
                const coverElement = element.querySelector(".radio-cover-image");
                coverElement.style.backgroundImage = `url(${this.coverImage})`;
            }
        }
        
        onViewCreated() {
            this.setClassList(['radio-conver-image-container']);
            this.setKey("cover-image");
        }
        ...
        render() {
            this.markup_builder({
                ...
                children: [
                    {
                        component: 'div',
                        classList: ['radio-cover-image', this.coverImage && this.coverImage.length > 0 ? "with-cover" : "default"],
                        key: "cover-image"
                    },
                ]
            });
    ...
    class RadioPlayerView extends CustomComponent /** implements ISoundBox */ {
        ...
        render() {
                                ...
                                children: [
                                    {
                                        ...
                                        children: [
                                            this.markupComponent(RadioCoverImage, {cover: this.getCoverUrl()}),
            ...
            const comp = this.getByKey("audio-cover-image");
            console.log(comp);
            if (comp && (this.getCoverUrl() && this.getCoverUrl().length > 0)) {
                const element = comp.getViewElement();
                element.style.backgroundImage = `url(${this.getCoverUrl()})`;
            }
