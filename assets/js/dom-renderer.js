...
((m) => {
    ...
    class View {
        ...
        _domBuilder(parent, docObj) {
            ...
            if  (comp instanceof HTMLImageComponent) {
                comp.src     = docObj.src;
                comp.loading = docObj.loading || "eager";
            } else if (comp instanceof HTMLSpanComponent)
                comp.setTextContent(docObj.textContent || '');
                
            const viewElement = comp.getViewElement();
            
            if ('events' in docObj) {
                for (const evntKey in docObj.events) {
                    for (const eventFn of docObj.events[evntKey]) {
                        viewElement.addEventListener(evntKey, (e) => {
                            if (typeof eventFn != "function")
                                return false;
                            try {
                                return eventFn(e, viewElement);
                            } catch(ex) {
                                console.log('Handle DOM Event Exception Caught for element', viewElement, 'stack trace: ', ex);
                            }
                        });
                    }
                }
            ...
            if ('attribs' in docObj) {
                for (const attrib in docObj.attribs) {
                    viewElement.setAttribute(attrib, docObj.attribs[attrib]);
                }
            }
    ...
    class GlobalRouting extends Module {
        ...
        static _routingInstance = null;
        
        constructor(routes) {
            super('GlobalRouting');
    
            GlobalRouting._routingInstance = this;
    ...
    class PageSwitcher extends Module {
        ...
        onPageSwitch(page, event) {
            ...
            let arr = this._listeners.get(page);
            if (!arr)
                this._listeners.set(page, arr = []);

            arr.push(event);
        }
        
        triggerEvent(event, page) {
            const events = this._listeners.get(page);
            if (!events)
                return;
            ...
            for (const evnt of events) {
                try {
                    ...
                    console.log("triggerEvent() exception: ", ex);
                }
            }
        }
        ...
        switch(page) {
            ...
            if (p) {
                if (this._activePage && this._pages.has(this._activePage)) {
                    this.triggerEvent("leaving", this._activePage);
                    this._pages.get(this._activePage).onPageSwitch();
                }
    
                this._activePage = page;
        
                this.triggerEvent("entering", page);
                p.onPageActive();
                return;
            }
    ...
    class DOMRenderer extends Module {
        ...
        renderComponent(c) {
            ...
            const compViewElement = c.getViewElement();
    
            if (c instanceof HTMLSpanComponent || c.innerText !== null)
                compViewElement.textContent = c.getTextContent();
    
            if (c instanceof HTMLImageComponent) {
                compViewElement.src     = c.src;
                compViewElement.loading = c.loading;
            }
        }
    ...
    wkInjectDeps(GlobalRouting, {

        getInstance: () => {
            return GlobalRouting._routingInstance;
        },
        
        getCurrentRoute: () => {
            let instance = GlobalRouting.getInstance();
            if (!instance)
                return null;
