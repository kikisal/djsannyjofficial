...
try {
    $stmt = $conn->prepare("SELECT `id`, `type`, `timestamp`, `image_url`, `audio_url`, `title`, `video_file_url` FROM `feeds` WHERE `type` = ? AND `timestamp` < ? ORDER BY `timestamp` DESC LIMIT $start, $itemCount");
